...
class enemy_bullet:
    ...
    def __init__(self, x1, y1, x2, y2, bullet_path):
        ...
        self.x2 = x2
        self.y2 = y2
        self.enemy_bullet_img = pygame.image.load(bullet_path)
    ...
    def bullet_diflect(self, collider):
        if self.y + self.col_r > collider[1] - collider[3] and self.y - self.col_r < collider[1] + collider[3] and self.x + self.col_r > collider[0] - collider[2] and self.x - self.col_r < collider[0] + collider[2]:
    ...
    def paint_bullet(self):
        Paint.screen.blit(self.enemy_bullet_img, (int(self.x) - self.r, int(self.y) - self.r))
        # pygame.draw.circle(Paint.screen, Paint.green, (int(self.x), int(self.y)), self.r)
